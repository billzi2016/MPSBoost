...
OvR 可以保留为兼容层和 fallback，但不得在文档、测试或发布说明中伪装成 native softmax。
...
- `softmax`：强制 native softmax。目标设备未实现 native softmax 时必须清晰失败，不允许静默伪装。
- `ovr`：显式使用 OvR compatibility layer。

当前阶段：

- CPU 多分类 `auto` 使用 native softmax。
- MPS native softmax 尚未完成时，不得把 CPU softmax 或 OvR 伪装成 MPS softmax。
...
- `predict_proba` 行归一；
- `GridSearchCV` 可调参；
- Iris / Digits / Covertype subset 至少覆盖一个默认 native softmax 路径；
