...
- [x] S0.3 建立本地 `AGENTS.md` 并加入 Git 忽略。
...
- [ ] S9.1 完成 `v2-arboretum-implementation/prd.md` 规格审查，确认 v2 范围、非目标和验收门。
- [ ] S9.2 设计统一树模型族抽象：目标函数、采样、split 策略、树生长和预测聚合。
- [ ] S9.3 保持现有回归 GBDT 行为与模型格式向后兼容。
- [ ] S9.4 建立模型族能力矩阵和未实现能力早失败错误。
- [ ] S9.5 明确 `0.3.0` all trees 发布范围，禁止为单个小模块频繁发布 PyPI。
- [ ] S9.6 验收 G8：新增抽象不复制数学公式，不引入第二套训练语义。
...
- [ ] S16.4 实现 feature importance：gain、split count 和 permutation。
