...
void ValidateViewImpl(const DenseMatrixView& view, std::uint32_t max_bins) {
  if (view.data == nullptr) {
    throw DataError("Input matrix data must not be null");
  }
  if (view.rows == 0 || view.features == 0) {
    throw DataError("Input matrix must contain at least one row and one feature");
  }
  if (max_bins < 2 || max_bins > 65536) {
    throw DataError("max_bins must be in [2, 65536]");
  }
  ...
  if (view.row_stride_bytes < item_size || view.column_stride_bytes < item_size) {
    throw DataError("Input matrix stride is smaller than the element size");
  }

  const std::uint64_t last_row = internal::CheckedMultiply(
      view.rows - 1, view.row_stride_bytes, "input matrix row stride");
  const std::uint64_t last_column = internal::CheckedMultiply(
      static_cast<std::uint64_t>(view.features - 1), view.column_stride_bytes,
      "input matrix column stride");
  const std::uint64_t last_offset = internal::CheckedAdd(
      internal::CheckedAdd(last_row, last_column, "input matrix maximum offset"),
      item_size, "input matrix final offset");
  internal::CheckedSize(last_offset, "input matrix maximum offset");

  const std::uint64_t value_count = internal::CheckedMultiply(
      view.rows, static_cast<std::uint64_t>(view.features), "input matrix element count");
  internal::CheckedSize(value_count, "binned element count");
}
...
float ReadFiniteOrMissingFloat(const DenseMatrixView& view,
  ...
  if (view.scalar_type == ScalarType::kFloat32) {
    ...
    if (!std::isfinite(value)) {
      throw DataError("Input matrix contains Inf");
    }
    return value;
  }
  double source = 0.0;
  ...
  if (source > kFloatMax || source < -kFloatMax) {
    throw DataError("float64 input value exceeds the finite float32 range");
  }
...
std::vector<float> BuildBoundaries(std::vector<float> values,
  ...
  boundaries.reserve(internal::CheckedSize(desired_bins - 1, "boundary count"));
  ...
  for (std::uint64_t part = 1; part < desired_bins; ++part) {
    ...
    const float candidate = values[internal::CheckedSize(rank, "quantile rank")];
...
BinnedDataset QuantizeDense(const DenseMatrixView& view, std::uint32_t max_bins) {
  ...
  const std::uint64_t value_count_u64 = internal::CheckedMultiply(
      view.rows, static_cast<std::uint64_t>(view.features), "binned element count");
  const std::size_t value_count = internal::CheckedSize(value_count_u64, "binned element count");
  ...
  std::vector<float> feature_values(internal::CheckedSize(view.rows, "single-feature workspace"));
  for (std::uint32_t feature = 0; feature < view.features; ++feature) {
    ...
    for (std::uint64_t row = 0; row < view.rows; ++row) {
      feature_values[internal::CheckedSize(row, "row index")] =
          ReadFiniteOrMissingFloat(view, row, feature);
      if (std::isnan(feature_values[internal::CheckedSize(row, "row index")])) {
        ++missing_count;
      }
    }
    ...
    for (std::uint64_t row = 0; row < view.rows; ++row) {
      const float value = feature_values[internal::CheckedSize(row, "row index")];
      ...
      const std::size_t output_index = internal::CheckedSize(
          static_cast<std::uint64_t>(feature) * view.rows + row, "binned output index");
...
BinnedDataset TransformDense(const DenseMatrixView& view,
  ...
  if (view.features != schema.features()) {
    throw DataError("Prediction input feature count does not match the binned schema");
  }
  ...
  const std::uint64_t value_count_u64 = internal::CheckedMultiply(
      view.rows, static_cast<std::uint64_t>(view.features), "prediction binned element count");
  const std::size_t value_count = internal::CheckedSize(value_count_u64,
                                                        "prediction binned element count");
  ...
  for (std::uint32_t feature = 0; feature < view.features; ++feature) {
    ...
    for (std::uint64_t row = 0; row < view.rows; ++row) {
      ...
      const std::size_t output_index = internal::CheckedSize(
          ...
          "prediction binned output index");
      result.missing_[output_index] = missing ? 1U : 0U;
      if (schema.storage() == BinStorage::kUInt8) {
        std::get<std::vector<std::uint8_t>>(result.bins_)[output_index] =
            static_cast<std::uint8_t>(bin);
...
std::uint32_t BinnedDataset::GetBin(std::uint64_t row,
  ...
  if (row >= rows_ || feature >= features()) {
    throw DataError("Binned index is out of bounds");
  }
  const std::size_t index = internal::CheckedSize(
      static_cast<std::uint64_t>(feature) * rows_ + row, "binned read index");
