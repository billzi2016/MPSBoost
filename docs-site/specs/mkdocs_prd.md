...
project documentation site that accurately presents current `1.0.0` capabilities
...
- Keep site content consistent with current `1.0.0` project state and append-only release history.
...
Documentation must reflect `1.0.0`:

- `mpsboost==1.0.0` is the current release target.
